...
This is an **iOS application currently being developed as an MVP (Minimum Viable Product)**.

The project intentionally has a **limited scope**. When implementing features, reviewing code, suggesting improvements, or making architectural decisions, prioritize:

* core product functionality,
* simplicity,
* fast iteration,
* maintainability appropriate for an MVP,
* only requirements that are explicitly needed for the current product scope.

Avoid introducing additional complexity, abstractions, edge-case handling, or platform features unless they are required by the current task.


## Accessibility

**Accessibility is explicitly out of scope for this MVP.**

Do **not** consider, implement, test, review, or suggest changes related to accessibility features or accessibility-specific interaction modes.

In particular, completely ignore requirements related to:

* keyboard navigation or hardware keyboard accessibility,
* Switch Control,
* Voice Control,
* accessibility-specific focus/navigation behavior,
* accessibility-specific interaction alternatives.

Do not raise accessibility concerns during code reviews and do not modify implementations solely to improve accessibility.

Unless a task explicitly states otherwise, assume that **accessibility requirements are intentionally excluded from the current MVP scope**.


## Core instructions

- Target iOS 17 or later. (Yes, it definitely exists.)
- Swift 5.x or later, using modern Swift concurrency. Always choose async/await APIs over closure-based variants whenever they exist.
