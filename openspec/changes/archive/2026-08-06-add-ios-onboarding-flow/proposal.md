...
- Complete the replaceable Looksmaxxing client against `https://api.micmac.io`: upload one prepared selfie, trigger and poll its analysis, search the `HAIRSTYLE` category and select its first ranked one-credit transformation, create and poll one generation, and map the annotated mesh, face shape, harmony score, generated image, and static style copy into the onboarding UI. Retain deterministic mock results, processing/error states, and the accessible before/after comparison.
...
- Pass the selected image through one observable in-memory flow boundary into mockable analysis and generation clients, retain remote request correlation only for the current process, never transmit questionnaire answers, and expose transport-independent harmony and generated-look results to the UI.
...
- Uses Firebase ID tokens for the configured Looksmaxxing API and a 10 MB selfie-upload ceiling. Defers Firebase project configuration, Google client/URL scheme, Apple capability/team setup, RevenueCat SDK key/identity policy/entitlement/offering/products/paywall, Looksmaxxing failure/refund/idempotency semantics and result lifetime, final legal URLs and facial-image disclosures, optimized-video encoding, and production icon/brand assets behind explicit configuration or adapter boundaries. Live facial-data upload and production release remain disabled until the final retention/deletion terms, disclosures, and legal URLs are approved.
